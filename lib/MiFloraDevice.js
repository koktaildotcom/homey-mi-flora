...
class MiFloraDevice extends Homey.Device {
    ...
    onSettings(oldSettingsObj, newSettingsObj, changedKeysArr, callback) {
        ...
        if (newSettingsObj.hasOwnProperty('identify')) {
            if (newSettingsObj.identify === true) {
                Homey.app.indentify(this).then(() => {
                    this.setSetting('identify', false)
                })
            }
        }
    }
    ...
    /**
     * Update the device when added
     */
    onAdded() {
        Homey.app.updateDevice(this).then(() => {
            console.log('The device is synced.');
        })
    }
}
